...
void dataTypes__int(void) {
  ...
  unsigned long long int_long_ulong =
      0xFFFFFFFFFFFFFFFF; // 8 bytes 0 to 18,446,744,073,709,551,615

  float float_float = .2;        // 4 bytes 1.2E-38 to 3.4E+38
  double float_double = 12.3203; // 8 bytes 1.7E-308 to 1.7E+308
  long double float_long_double =
      1233.12313; // 16 bytes  3.4E-4932 to 1.1E+4932
...
void dataTypes__typeCasting(void) {
  ...
  // Warning!!!
  // Pointer conversions -->
  // https://learn.microsoft.com/en-us/cpp/cpp/type-conversions-and-type-safety-modern-cpp?view=msvc-170#pointer-conversions
  /*
  In many expressions, a C-style array is implicitly converted to a pointer to
  the first element in the array, and constant conversions can happen silently.
  Although this is convenient, it's also potentially error-prone. For example,
  the following badly designed code example seems nonsensical, and yet it will
  compile and produces a result of 'p'. First, the "Help" string constant
  literal is converted to a char* that points to the first element of the array;
    that pointer is then incremented by three elements so that it now points to
  the last element 'p'.
...
void dataTypes__stringInterpolation(void) {
  ...
  // respectively for values different than zero. Used with a, A, e, E, f, F, g
  // or G it forces the written output to contain a decimal point even if no
  // more digits follow. By default, if no digits follow, no decimal point is
  // written. .numbeber : preciosion
